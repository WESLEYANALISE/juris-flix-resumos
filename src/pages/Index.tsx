...
const Index = () => {
    ...
    return (
      <div className="min-h-screen bg-netflix-black flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-netflix-lightGray animate-pulse">Carregamento ultra-rápido...</p>
        </div>
      </div>
    );
  }
  ...
  const handleSubjectClick = (area: string, modulo: string, tema: string, assunto: string, assuntoId: number) => {
    ...
    if (assuntoData) {
      // Adiciona aos recentes automaticamente
    ...
    switch (viewState.type) {
      ...
      case 'assuntos':
        ...
        return (
          <div className="space-y-6">
            <button 
              onClick={() => setViewState({ type: 'temas', area: viewState.area, numeroModulo: viewState.numeroModulo, nomeModulo: viewState.nomeModulo })} 
              className="text-netflix-red hover:text-netflix-darkRed transition-colors font-medium"
            >
              ← Voltar para temas
            </button>
  ...
  return (
    <div className="min-h-screen bg-netflix-black">
      {showNavigation && (
        <div className="container mx-auto py-8 max-w-7xl px-[10px]">
          {showHeader && (
