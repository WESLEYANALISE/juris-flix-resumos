...
type ViewState = 
  | { type: 'areas' }
  | { type: 'modulos'; area: string }
  | { type: 'temas'; area: string; numeroModulo: string; nomeModulo: string }
  | { type: 'assuntos'; area: string; numeroModulo: string; nomeModulo: string; numeroTema: string; nomeTema: string }
  | { type: 'resumo'; area: string; numeroModulo: string; nomeModulo: string; numeroTema: string; nomeTema: string; assunto: string; resumo: string; glossario: string; assuntoId: number };
...
const Index = () => {
  ...
  const { loading, error, favorites, recents, getAreas, getModulosByArea, getTemasByModulo, getAssuntosByTema } = useResumos();
      ...
      case 'areas':
        ...
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {areas.map(({ area, modulosCount }) => (
              <AreaCard
                key={area}
                area={area}
                temasCount={modulosCount}
        ...
        return (
          <div className="min-h-screen">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {modulos.map(({ numero, nome, temasCount }) => (
                <ModuloCard
                  key={`${numero}-${nome}`}
                  numero={numero}
                  nome={nome}
